...
Slackで川柳（5-7-5）・短歌（5-7-5-7-7）・七七（7-7の下の句）を検出します。メッセージに該当する音のまとまりを見つけると、そのスレッドに返信します（例:「川柳を検出しました！」「短歌を検出しました！」「七七を検出しました！」）。長い音律から順に判定するため、短歌は5-7-5として誤検出されません。
...
「川柳を検出しました！」と返信されます。「寒いねと話しかければ寒いねと答える人のいる暖かさ」のような
5-7-5-7-7なら「短歌を検出しました！」、7-7の下の句なら「七七を検出しました！」と返信します。
なお7-7は短く誤検出しやすいため、メッセージ全体がほぼ7-7だけで構成される場合にのみ検出します
（通常の会話に含まれる7-7の断片には反応しません）。
